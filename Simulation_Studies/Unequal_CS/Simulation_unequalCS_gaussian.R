...
set.seed(seed)
# We will focus on closed cohort at subcluster level and cross-sectional at individual level (no sig2_g)
...
simData <- foreach(i=1:niteration, .combine=rbind) %dopar% {
  ...
  m1<-lmer(y~factor(period)+treatment+(1|clust)+(1|clinic)+(1|cluster.period)+(1|clinic.period),data=dt2,REML=T)
