...
# define scenarios
scenarios <- read.table("parameters_gaussian_type1error.txt", header=TRUE, sep="")
...
paste("n=",n,"l=",l,"m=",m,"t=",t,"delta=",delta,"bs=",bs,"niteration=",niteration,"seed=",seed,"tot.var=",tot.var)
...
sig2_e<-tot.var*(1-alpha[1])
sig2_b<-tot.var*alpha[3]                                              #within cluster
sig2_c<-tot.var*(alpha[4]-alpha[3])                                   #within clinic
sig2_s<-tot.var*(alpha[2]-alpha[3])                                   #within cluster within period
sig2_p<-tot.var*(alpha[1]-alpha[4]-alpha[2]+alpha[3])                 #within clinic within period
...
simData <- foreach(i=1:niteration, .combine=rbind) %dopar% {
    ...
    for(j in 1:t){
      for(h in 1:l){
        for(k in 1:m){
          ...
          y.ijlk <- beta[j] + delta*treatment.ijlk + b_i + c_il[h] + s_ij[j] + p_ijl[h,j] + rnorm(1,mean=0,sd=sqrt(sig2_e))
  ...
  m1<-lmer(y~factor(period)+treatment+(1|clust)+(1|clinic)+(1|cluster.period)+(1|clinic.period),data=dt2,REML=F)
...
write.table(simData, file=paste("results/GaussResults_",analysis,"_scenario",scenario,".txt",sep=""), sep="\t", row.names=F)
